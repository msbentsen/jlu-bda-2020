...
def parse():
	...
	data = str(os.path.dirname(os.path.abspath(__file__)).replace('bin/scripts','data'))
	...
	for genome in genomes:
		...
		for biosource in biosources:
			...
			for tf in tfs:
				...
				for f in files:
					if f.split('.')[1] == 'bed':
						tf_bed_dict.update(read_bed(data + '/' + genome + '/' + biosource + '/ChIP-seq/' + tf + '/' + f))
			...
			for f in os.listdir(data + '/' + genome + '/' + biosource + '/ATAC-seq'):
				if f.split('.')[1] == 'bigWig':
					...
					with open(data + '/' + 'pickledata/' + genome + '/ATAC-seq/' + biosource + '.pickle', 'wb') as handle:
